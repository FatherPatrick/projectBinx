...
const ensurePollSchema = async () => {
  ...
  await pool.query(`ALTER TABLE polls ADD COLUMN IF NOT EXISTS created_by VARCHAR(120);`);
  await pool.query(`ALTER TABLE polls ADD COLUMN IF NOT EXISTS description TEXT;`);
  await pool.query(`ALTER TABLE polls ADD COLUMN IF NOT EXISTS poll_type VARCHAR(30);`);
  await pool.query(`ALTER TABLE polls ADD COLUMN IF NOT EXISTS user_name VARCHAR(120);`);
  await pool.query(`ALTER TABLE polls ADD COLUMN IF NOT EXISTS question TEXT;`);
  await pool.query(`ALTER TABLE polls ADD COLUMN IF NOT EXISTS type VARCHAR(30);`);
  await pool.query(
  ...
  await pool.query(`DROP INDEX IF EXISTS ux_poll_options_poll_id_option_index;`);
};
...
const seedDefaultPolls = async () => {
  const existingPollCount = await pool.query('SELECT COUNT(*)::int AS count FROM polls');
